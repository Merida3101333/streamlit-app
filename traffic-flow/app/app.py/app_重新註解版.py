...
#   - 區域中心值標籤
#   - 全域中心/縮放控制（4 張地圖共用）
#   - 區域彙總：Origin / Destination / OD
#   - 標籤顯示採「四捨五入 half-up」（Sum: 0 位；Mean: 1 位）
# =========================================
...
# ---------- 路徑 ----------
ROOT = Path(r"C:\traffic-flow")   # ← 請改成你的專案根目錄
...
def guess_center(geojson):
    try:
        ...
        pass
    return 40.758, -73.985  # 預設 Times Square
...
# ---------- 共用地圖生成 ----------
# 新增 label_digits 控制標籤小數位（Sum=0；Mean=1）
...
def hourly_zone_values(df_obs_hourly: pd.DataFrame, day_abs: int, hours: list[int],
    ...
    """
    參數：
      df_obs_hourly: obs_hourly_canonical.csv 載入後的 DataFrame
      day_abs      : 第幾天 (1..252)
      hours        : 小時清單（如 [1,2,3]）
      agg_mode     : Origin / Destination / OD
      stat         : 對小時做 Sum 或 Mean
    回傳：index=zone_id 的 Series
    """
...
# ---- 讀取 LSTM 每小時原始檔並彙整到每區（目前未在 Tab 使用，保留）----
...
if use_fit:
    # Folium 初次渲染才可 fit；這裡以固定 center/zoom 模擬
...
with tab1:
    ...
    # OBS / ARIMA / TensorTS / LSTM 依 stat1 做單位一致化（不改原始數值）
...
with tab2:
    ...
    # 左：區間聚合（Sum/Mean over hours_range）
    with left:
