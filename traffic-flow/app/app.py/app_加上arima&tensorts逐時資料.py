...
with tab1:
    ...
    with colc3:
        agg_mode = st.selectbox("Aggregate to zone", ["Origin sum (flow-out)","Destination sum (flow-in)","OD sum"], index=0)
    with colc4:
        st.write("")  # 占位

    # 四組值
    v_obs    = period_zone_values("obs",     day_21, period, agg_mode)
    v_arima  = period_zone_values("arima",   day_21, period, agg_mode)
    v_tensor = period_zone_values("tensor",  day_21, period, agg_mode)   # ← 改為 "tensor"
